...
violations = sc.textFile('Data/ParkingViolationCodes.txt').map(lambda line: line.split('\t')).filter(lambda line: 'VIOLATION CODE' not in line).map(lambda line: (line[0], line[2]))
violation_info = {}
